                       ...
                       APIType api_type, bool stereo)
{
...

  {
    ...
    else
    {
      out.Write("\twrappedcoord.x = fixpoint_uv{}.x & ({} - 1);\n", texcoord,
                tev_ind_wrap_start[static_cast<u32>(tevind.sw.Value()) -
                                   static_cast<u32>(IndTexWrap::ITW_256)]);
    }
    ...
    else
    {
      out.Write("\twrappedcoord.y = fixpoint_uv{}.y & ({} - 1);\n", texcoord,
                tev_ind_wrap_start[static_cast<u32>(tevind.tw.Value()) -
                                   static_cast<u32>(IndTexWrap::ITW_256)]);
    }
  ...
  if (cc.dest >= TevOutput::Color0)
    out.SetConstantsUsed(C_COLORS + static_cast<u32>(cc.dest.Value()),
                         C_COLORS + static_cast<u32>(cc.dest.Value()));

  if (ac.dest >= TevOutput::Color0)
    out.SetConstantsUsed(C_COLORS + static_cast<u32>(ac.dest.Value()),
                         C_COLORS + static_cast<u32>(ac.dest.Value()));
