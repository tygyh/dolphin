...
public:
  ...
  constexpr NormalizedFloat() noexcept : value_(T(0)) {}

  // Constructs from a raw value, clamping into [0, 1].
  explicit constexpr NormalizedFloat(T v) noexcept : value_(std::clamp(v, T(0), T(1))) {}

  // Converts to T via division by maxValue, then clamps into [0, 1].
  template <typename I, std::enable_if_t<std::is_integral_v<I>, int> = 0>
  explicit constexpr NormalizedFloat(I v, I maxValue) noexcept
      : value_(std::clamp(static_cast<T>(v) / static_cast<T>(maxValue), T(0), T(1)))
  {
  }
...
public:
  // Default constructs to 0
  constexpr UnitFloat() noexcept : value_(T(0)) {}
  ...
  // Constructs from a raw value, clamping into [-1, 1].
  template <typename I, std::enable_if_t<std::is_integral_v<I>, int> = 0>
  explicit constexpr UnitFloat(I v, I maxMagnitude) noexcept
      : value_(std::clamp(static_cast<T>(v) / static_cast<T>(maxMagnitude), T(-1), T(1)))
  {
  }
